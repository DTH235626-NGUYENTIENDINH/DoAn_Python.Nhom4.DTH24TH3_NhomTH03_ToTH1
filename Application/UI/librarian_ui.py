...
from ..Handle import handle_librarian as hl
...
def librarian_ui():
    ...
    tabControl.add(tab_ReaderManagement, text='Quản Lý Đọc Giả')
    ...
    #Tạo list lưu entry để lấy dữ liệu
    book_entry = []
    book_labels = ["Tên sách:", "Tác giả:", "Thể loại:", "Số lượng:"]
    for i, label_text in enumerate(book_labels):
        entry = ttk.Entry(info_frame_book, width=50)
        entry.grid(row=i, column=1, padx=5, pady=5, sticky="ew")
        book_entry.append(entry)  
    info_frame_book.grid_columnconfigure(1, weight=1) 
    # Gửi danh sách các ô Entry cho file handle để nó lưu lại
    hl.register_book_entries(book_entry)    
    ...
    # Gửi Treeview cho file handle để nó lưu lại
    hl.register_book_treeview(tree_book)
    # Load dữ liệu sách từ database và hiển thị lên Treeview
    hl.load_books_data()
    # Gán sự kiện chọn dòng trong Treeview để hiển thị thông tin lên Entry
    tree_book.bind("<<TreeviewSelect>>", hl.on_book_select)
    #===================================================================
    #---GIAO DIỆN 2: QUẢN LÝ ĐỌC GIẢ (Nội dung cho tab_ReaderManagement)---
    #===================================================================
    #Khung nhập thông tin đọc giả
    lbl_TextREADER = ttk.Label(tab_ReaderManagement, text="QUẢN LÝ ĐỌC GIẢ", font=("Arial", 20, 'bold'), foreground="#09611C")
    lbl_TextREADER.pack(pady=10)
    info_frame_reader = ttk.LabelFrame(tab_ReaderManagement, text="Thông tin đọc giả")
    ...
    columns_reader = ("Mã đọc giả", "Họ và Tên", "Username", "Số điện thoại")
    ...
    borrow_labels = ["Mã đọc giả:", "Mã sách:", "Ngày mượn:", "Ngày hẹn trả:", "Trạng thái:"]
    ...
    columns_borrow = ("Mã phiếu mượn", "Mã đọc giả", "Mã sách", "Ngày mượn", "Ngày hẹn trả", "Trạng thái")
